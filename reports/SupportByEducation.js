...
function buildSQL(campaignId) {
    ...
    sql += `    , CASE WHEN SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(hs_and_under_supportive) / SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed))\r\n`
    sql += `      ELSE 0 END hs_and_under_supportive_perc\r\n`

    sql += `    , CASE WHEN SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(hs_and_under_neutral) / SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed))\r\n`
    sql += `      ELSE 0 END hs_and_under_neutral_perc\r\n`

    sql += `    , CASE WHEN SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(hs_and_under_opposed) / SUM(hs_and_under_supportive + hs_and_under_neutral + hs_and_under_opposed))\r\n`
    sql += `      ELSE 0 END hs_and_under_opposed_perc\r\n`    

    sql += `    , CASE WHEN SUM(some_college_supportive + some_college_neutral + some_college_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(some_college_supportive) / SUM(some_college_supportive + some_college_neutral + some_college_opposed))\r\n`
    sql += `      ELSE 0 END some_college_supportive_perc\r\n`

    sql += `    , CASE WHEN SUM(some_college_supportive + some_college_neutral + some_college_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(some_college_neutral) / SUM(some_college_supportive + some_college_neutral + some_college_opposed))\r\n`
    sql += `      ELSE 0 END some_college_neutral_perc\r\n`

    sql += `    , CASE WHEN SUM(some_college_supportive + some_college_neutral + some_college_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(some_college_opposed) / SUM(some_college_supportive + some_college_neutral + some_college_opposed))\r\n`
    sql += `      ELSE 0 END some_college_opposed_perc\r\n`

    sql += `    , CASE WHEN SUM(ba_supportive + ba_neutral + ba_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ba_supportive) / SUM(ba_supportive + ba_neutral + ba_opposed))\r\n`
    sql += `      ELSE 0 END ba_supportive_perc\r\n`

    sql += `    , CASE WHEN SUM(ba_supportive + ba_neutral + ba_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ba_neutral) / SUM(ba_supportive + ba_neutral + ba_opposed))\r\n`
    sql += `      ELSE 0 END ba_neutral_perc\r\n`

    sql += `    , CASE WHEN SUM(ba_supportive + ba_neutral + ba_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ba_opposed) / SUM(ba_supportive + ba_neutral + ba_opposed))\r\n`
    sql += `      ELSE 0 END ba_opposed_perc\r\n`

    sql += `    , CASE WHEN SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ma_plus_supportive) / SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed))\r\n`
    sql += `      ELSE 0 END ma_plus_supportive_perc\r\n`

    sql += `    , CASE WHEN SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ma_plus_neutral) / SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed))\r\n`
    sql += `      ELSE 0 END ma_plus_neutral_perc\r\n`

    sql += `    , CASE WHEN SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed) > 0\r\n`
    sql += `      THEN ROUND(100 * SUM(ma_plus_opposed) / SUM(ma_plus_supportive + ma_plus_neutral + ma_plus_opposed))\r\n`
    sql += `      ELSE 0 END ma_plus_opposed_perc\r\n`

    sql += `FROM (\r\n`
    sql += `    SELECT act.supportive, act.neutral, act.opposed\r\n`

    sql += `    , CASE WHEN edu.est_schooling_total_pop > 0\r\n`
    sql += `      THEN act.supportive * CAST(\r\n`
    ...
    sql += `      ELSE 0 END hs_and_under_supportive\r\n`

    sql += `    , CASE WHEN edu.est_schooling_total_pop > 0\r\n`
    sql += `      THEN act.neutral * CAST(\r\n`
    sql += `        edu.est_male_no_schooling_pop + edu.est_male_nursery_to_4th_grade_pop + edu.est_male_5th_and_6th_pop + edu.est_male_7th_and_8th_pop + edu.est_male_9th_pop + edu.est_male_10th_pop + edu.est_male_11th_pop + edu.est_male_12th_no_diploma_pop + edu.est_male_hs_grad_pop\r\n`
    sql += `        + edu.est_female_no_schooling_pop + edu.est_female_nursery_to_4th_grade_pop + edu.est_female_5th_and_6th_pop + edu.est_female_7th_and_8th_pop + edu.est_female_9th_pop + edu.est_female_10th_pop + edu.est_female_11th_pop + edu.est_female_12th_no_diploma_pop + edu.est_female_hs_grad_pop as decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END hs_and_under_neutral\r\n`

    sql += `    , CASE WHEN edu.est_schooling_total_pop > 0\r\n`
    sql += `      THEN act.opposed * CAST(\r\n`
    sql += `        edu.est_male_no_schooling_pop + edu.est_male_nursery_to_4th_grade_pop + edu.est_male_5th_and_6th_pop + edu.est_male_7th_and_8th_pop + edu.est_male_9th_pop + edu.est_male_10th_pop + edu.est_male_11th_pop + edu.est_male_12th_no_diploma_pop + edu.est_male_hs_grad_pop\r\n`
    sql += `        + edu.est_female_no_schooling_pop + edu.est_female_nursery_to_4th_grade_pop + edu.est_female_5th_and_6th_pop + edu.est_female_7th_and_8th_pop + edu.est_female_9th_pop + edu.est_female_10th_pop + edu.est_female_11th_pop + edu.est_female_12th_no_diploma_pop + edu.est_female_hs_grad_pop as decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END hs_and_under_opposed\r\n`
    ...
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END some_college_supportive\r\n`
    ...
    sql += `        + edu.est_female_less_than_1_year_college_pop + edu.est_female_some_college_no_degree_pop + edu.est_female_associates_degree_pop as decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END some_college_neutral\r\n`
    ...
    sql += `        + edu.est_female_less_than_1_year_college_pop + edu.est_female_some_college_no_degree_pop + edu.est_female_associates_degree_pop as decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END some_college_opposed\r\n`
    ...
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END ma_plus_supportive\r\n`
    ...
    sql += `        + edu.est_female_masters_degree_pop + edu.est_female_professional_degree_pop + edu.est_female_doctorate_degree_pop AS decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END ma_plus_neutral\r\n`
    ...
    sql += `        + edu.est_female_masters_degree_pop + edu.est_female_professional_degree_pop + edu.est_female_doctorate_degree_pop AS decimal)\r\n`
    sql += `        / edu.est_schooling_total_pop\r\n`
    sql += `      ELSE 0 END ma_plus_opposed\r\n`
