...
//====================================================================================
//
// DonationPledgeList report
//
// Returns the list of contacts that have pledged a donation
// 
//====================================================================================
router.get('/DonationPledgeList', utils.authenticateToken, async(req, res) => {
    ...
    try {
        ...
        .catch( (e) => {
            ErrorRecorder.recordAppError(new AppError('data-server', 'routes/reports.js', 'GET /DonationPledgeList', 'Unknown error in GET /', e))
            throw e
        })
        ...
        // Generate and return the report
        try {
            const report = require('../reports/DonationPledgeList')
            return res.status(200).send( await report(campaignId, targetActorId, dateStart, dateEnd) )
        } catch (e) {
            return res.status(400).send(`Error generating Donation Pledge List report: ${e.message}`)
        }

    } catch(e) {
        ErrorRecorder.recordAppError(new AppError('data-server', 'routes/reports.js', 'GET /DonationPledgeList', 'Unknown error in GET /DonationPledgeList', e))
        throw e
    }
});
